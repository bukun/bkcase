...
def get_file_list(secfile):
    ...
    for cnt in cnts:
        if cnt.startswith('\subsection'):
            # subsec的文件名
            subsec_name = 'sub%s.tex' % (str(dirindex).zfill(3))
            dirindex += 10
...
if __name__ == '__main__':
    ...
    for xwroot, xwdirs, xwfiles in os.walk(inws):
        for secfile in xwfiles:
            if secfile.startswith('sec') and    secfile.endswith('tex'):
                secfile = os.path.join(xwroot, secfile)
